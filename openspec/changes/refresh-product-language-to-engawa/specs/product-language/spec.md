...
システムは、撮影した写真と記録から item を作るユーザー行為を「写しを収める」、未決定の item（内部 `status='candidate'`）が身を置くuser-facingな場所を「縁側」、itemを記録を保持したまま `status='discarded'` へ移す変容を「捨離する」と SHALL 呼び分ける。これらを一語へ機械的に統合してはならない（MUST NOT）。
...
- **THEN** アクションと結果は「捨離する」「捨離済み」と表現され、「手放す」「手放し済み」を主要ラベルとして表示しない
...
- **WHEN** ユーザーが捨離済み画像を端末フォトライブラリへコピーする
