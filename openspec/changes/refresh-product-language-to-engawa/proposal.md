...
現在の「退避」は、撮影後の行為、未決定の置き場所、完了演出、取り消しまでを一語で担っており、機能は伝わる一方で不断捨離の世界観を十分に表せていない。「候補」「手放す」も説明的なままで、青い開いた円相と朱の閉じた円相が示す体験に対して言葉が追いついていない。

撮影した記録を作る行為を「写しを収める」、捨てる／残すを決めないモノの居場所を「縁側」、決断後の変容を「捨離」と呼び分け、わかりやすさを保ったままプロダクト固有の語彙体系へ刷新する。
...
- user-facing な手放し操作と手放し済み状態（内部 `status='discarded'`）を **「捨離する」「捨離済み」** に統一する。
- 青い開いた円相を「写しは収まったが、判断はまだ開いている縁側」、グレー円相を捨離確認、朱の閉じた円相を捨離完了として意味づけ直す。円相アセット、色、アニメーション自体は変更しない。
...
- `release`: 「手放す」を「捨離する」へ、候補戻しを「もう一度、縁側へ」へ変更する。
- `enso-motif`: 青・灰・朱の円相を新しい語彙体系へ対応づける。

## Impact

- UI: `CaptureScreen`, `EvacuationCompleteScreen`, `EmptyState`, `BottomNav`, `CandidateListScreen`, `ItemDetailScreen`, `ReleaseConfirmScreen`, `ReleaseCompleteScreen`, `DiscardedListScreen`, `DiscardedDetailScreen`, `ItemCard`, `ItemListRow`。
- 設定: `app.json` のカメラ／写真権限説明。ネイティブ権限文言の反映には新しいアプリバイナリが必要。
